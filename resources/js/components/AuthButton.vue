<template>
  <el-button :type="type_data" :size="size_data" :disabled="is_disable" :loading="is_loading" @click="handleClick" size="small" plain v-if="isShow">{{text}}</el-button>
</template>
...
export default {
  ...
  setup(props, context) {
    ...
    let {text, type,size,route_name,isDisable,isLoading} = toRefs(props); // 将props转化为响应式，因为 props 是响应式的，你不能使用 ES6 解构，因为它会消除 prop 的响应性。
