...
class Utils
{
     ...
     * 生成随机code (Day)
     */
    public static function generateRandCodeByDay(string $prefix = '', string $type = 'rc', int $length = 6): string
    {
        ...
        return $prefix.$currentCycle.strval(str_pad($codeNum, $length, '0', STR_PAD_LEFT));
    }

    /**
     * 生成随机code (Month)
     */
    public static function generateRandCodeByMonth(string $prefix = '', string $type = 'rc', int $length = 6): string
    {
        $Redis = Cache::store('redis')->handler();
     ...
     * 生成随机code (Year)
     */
    public static function generateRandCodeByYear(string $prefix = '', string $type = 'rc', int $length = 6): string
    {
        $Redis = Cache::store('redis')->handler();
